"""
Legend-aware sweep:
  * Per PDF, read its legend → set of symbols present (subset of {5АЭ, 6АЭ, 7АЭ}).
  * Run shape detector with templates_curated_v2.
  * Re-route the AMBIGUOUS square cluster (W=H≈9.6, n≈80, AR≈1.0):
      - if only 5АЭ in legend → count as 5АЭ
      - if only 7АЭ in legend → count as 7АЭ
      - if both                → keep raw NCC winner
      - if neither             → drop (likely something else)
  * 5АЭ_h is always 5АЭ (horizontal arrow is unique).
  * 6АЭ stays as-is.
"""
...
OUT = ROOT / "_sweep_legend_aware_out"
...
CLASSES = ["5АЭ", "6АЭ", "7АЭ"]
AMBIG_LABEL = {"5АЭ", "7АЭ"}  # interchangeable square shape
...
# ---------- Detector with legend awareness ----------
def detect_pdf(pdf_path: Path):
    ...
    # routing rule for the ambiguous square label:
    present_in_ambig = AMBIG_LABEL & legend_syms
    if len(present_in_ambig) == 1:
        ambig_redirect = next(iter(present_in_ambig))   # only one of {5АЭ, 7АЭ}
    else:
        ambig_redirect = None  # both or none → keep raw winner / drop
    ...
    for cl in cls:
        ...
        # alias: 5АЭ_h → 5АЭ
        if INFO[best_lab].get("_alias_for"):
            target = INFO[best_lab]["_alias_for"]
        else:
            target = best_lab

        # ambiguous square redirect
        if target in AMBIG_LABEL:
            if ambig_redirect is not None:
                target = ambig_redirect
            elif not present_in_ambig:
                continue   # neither 5АЭ nor 7АЭ in legend — drop
            # else (both present) keep raw target
...
(OUT/"sweep_legend_aware.json").write_text(
    json.dumps({"per_pair":results,"aggregate":agg},ensure_ascii=False,indent=2),
    encoding="utf-8")
print(f"\nSaved: {OUT/'sweep_legend_aware.json'}")

"""
Accuracy sweep for the 7 lighting plans (005..011).

For each (PDF, DXF) pair:
  * GT  : counts from equipment_counter.py on the DXF (5AE/6AE/7AE)
  * DET : counts from the v3 shape-aware detector on the PDF
          (NCC vs templates_curated_v2 + AR/n-parts gates)

Writes:
  _sweep_lighting_out/sweep_results.json
  _sweep_lighting_out/sweep_summary.md
"""
...
# detector params (mirror _detect_shape_v3_005.py)
TPL_SIZE   = 64
LINK_DIST  = 5.0
MAX_PRIM   = 60.0
NCC_MIN    = 0.25
...
# ---------- DXF GT via equipment_counter.py subprocess ----------

GT_RE = re.compile(r"^\s*\d+\s+(\S+)\s+.+?\s+(\d+)(?:\s+(\d+))?(?:\s+(\d+))?\s*$")

def collect_gt(dxf_path: Path) -> dict[str, int]:
    """Run equipment_counter.py on the DXF, parse rows, return per-class total counts."""
    ...
    for line in out.splitlines():
        ...
        if sym in counts:
            # last number on a row in equipment_counter is "Всего" if present, else "Кол-во"
            counts[sym] += nums[-1]
    return counts

# ---------- Detector (v3 algorithm) ----------
...
for lab in manifest:
    ...
    if not npy.exists():
        print(f"WARN: missing template {npy}"); continue
...
def ar_window(ar, info):
    if info["ar_lo"] <= ar <= info["ar_hi"]: return True
    if info["ar_lo_rotated"] <= ar <= info["ar_hi_rotated"]: return True
    return False

def detect_pdf(pdf_path: Path) -> dict[str, int]:
    try:
    ...
    counts = {c: 0 for c in CLASSES}
    # alias: any "<class>_h" template counts toward "<class>"
    def alias_of(lab: str) -> str:
        info = INFO.get(lab, {})
        return info.get("_alias_for", lab)
    for cl in cls:
        ...
        if best_sc >= NCC_MIN and (best_sc - second_sc) >= MARGIN_MIN:
            target = alias_of(best_lab)
            if target in counts:
                counts[target] += 1
    doc.close()
    return counts
...
results = []
print()
print(f"{'File':<10} | {'Class':<5} | {'GT':>4} | {'DET':>4} | {'diff':>5}")
print("-"*44)
for pdf_name, dxf_name in PAIRS:
    ...
    if not pdf_path.exists() or not dxf_path.exists():
        print(f"  MISSING: {pdf_name} or {dxf_name}")
        continue
    print(f"\n[{tag}] GT…", end=" ", flush=True)
    gt = collect_gt(dxf_path)
    print(f"GT={gt}", end=" ; ", flush=True)
    print("DET…", end=" ", flush=True)
    det = detect_pdf(pdf_path)
    print(f"DET={det}")
    for c in CLASSES:
        g, d = gt[c], det[c]
        diff = (d - g) if (g >= 0 and d >= 0) else None
        results.append({"file": tag, "class": c, "gt": g, "det": d, "diff": diff})
        print(f"  {tag:<10} | {c:<5} | {g:>4} | {d:>4} | "
...
for r in results:
    ...
    agg[c]["gt_tot"]  += r["gt"]
    agg[c]["det_tot"] += r["det"]
    agg[c]["tp"]      += min(r["gt"], r["det"])     # crude lower bound

print("\n=== Aggregate (all 7 plans) ===")
for c in CLASSES:
    g, d, tp = agg[c]["gt_tot"], agg[c]["det_tot"], agg[c]["tp"]
    prec = tp/d if d else 0.0
    rec  = tp/g if g else 0.0
    print(f"  {c}: GT={g:>3} DET={d:>3}  P~{prec:.2f}  R~{rec:.2f}")

# ---------- Persist ----------

(OUT/"sweep_results.json").write_text(
    json.dumps({"per_pair": results, "aggregate": agg,
                "thresholds": {"NCC_MIN": NCC_MIN, "MARGIN_MIN": MARGIN_MIN,
                               "MIN_PARTS": MIN_PARTS, "LINK_DIST": LINK_DIST}},
               ensure_ascii=False, indent=2),
    encoding="utf-8")

md = ["# Lighting plan accuracy sweep (005–011)", "",
      ...
      "|------|-------|---:|---------:|-----:|"]
for r in results:
    diff_s = "—" if r["diff"] is None else f"{r['diff']:+d}"
    md.append(f"| {r['file']} | {r['class']} | {r['gt']} | {r['det']} | {diff_s} |")
md += ["", "## Aggregate (all plans)", "",
...
for c in CLASSES:
    ...
    prec = tp/d if d else 0.0
    rec  = tp/g if g else 0.0
    md.append(f"| {c} | {g} | {d} | {prec:.2f} | {rec:.2f} |")
(OUT/"sweep_summary.md").write_text("\n".join(md) + "\n", encoding="utf-8")
print(f"\nSaved: {OUT/'sweep_results.json'}")
print(f"Saved: {OUT/'sweep_summary.md'}")
